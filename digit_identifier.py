...
def modify_digits_in_text(text):
    # Use regular expression to find all sequences of digits with commas or points in the text
    modified_text = re.sub(r'\b(\d{1,3}(,?\d{3})*(\.?\d+)?)\b', r'<\1>', text)

    # Find all occurrences of text within < and > and remove commas within them
    modified_text = re.sub(r'(<[^<>]+>)', lambda match: match.group(0).replace(',', ''), modified_text)

    # Combine words after removing commas within <>
